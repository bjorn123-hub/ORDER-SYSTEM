...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['item'])) {
    ...
    if (array_key_exists($item, $menuItems)) {
        ...
        if (!isset($_SESSION['cart'])) {
            $_SESSION['cart'] = [];
            $_SESSION['delivery_time'] = []; // Initialize delivery time array
        }
        ...
        if (isset($_SESSION['cart'][$item])) {
            ...
            $_SESSION['delivery_time'] = date("H:i", strtotime("+30 minutes"));
        }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cancel_order'])) {
    ...
    unset($_SESSION['delivery_time']); // Clear delivery time
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['receive_order'])) {
    // Clear the cart and delivery time
    unset($_SESSION['cart']);
    unset($_SESSION['delivery_time']);
    $_SESSION['message'] = "Thank you! Your order has been received."; // Confirmation message
    header('Location: menu.php');
    exit();
}
...
<body>
    ...
    <p>Total Price: $<?php echo number_format(calculateTotalPrice(), 2); ?></p>
    <p>Estimated Delivery Time: <?php echo isset($_SESSION['delivery_time']) ? $_SESSION['delivery_time'] : 'N/A'; ?></p>
</div>

                <form action="menu.php" method="POST">
                    <button type="submit" name="cancel_order" class="cancel-order-btn">Cancel Order</button>
                    <button type="submit" name="receive_order" class="receive-order-btn">Receive Order</button>
                </form>
            <?php else: ?>
